    ...
    sync: {
        ...
        writeSyncData: (data) => {
            ...
            try{
                ...
                console.error('Can not write data to storage. Bad json data!');
            }
            try{
                // Fav
                ...
                if(data.login !== null && data.login !== ''){
                    ...
                    if(data.login != null && data.login != '') {
                        if (localLogin == null || localLogin == '') {
                            ...
                            // Write missing data
                            for(let user in data.login) {
                                let valid = true;
                                for(let item in localLogin) {
                                    if(localLogin[item].id == data.login[user].id) {
                                        valid = false;
                                    }
                                }
                                if (valid) {
                                    localLogin.push(data.login[user]);
                                }
                            }
                ...
                // Login
                if(data.fav !== null && data.fav !== ''){
                    let localFav = [ myRadio.getFavList() ]; // TODO: aßen klammern waren fehler
                    // Write full data
                    if(data.fav != null && data.fav != ''){
                        if(localFav == null || localFav == ''){
                            localStorage.setItem('favorites', JSON.stringify(data.fav));
                        }else{
                            // Write missing data
                            for(let fav in data.fav) {
                                let check = true;
                                let valid = false;
                                for (let item in localFav) {
                                    if(localFav[item].id == data.fav[fav].id) {
                                        if (localFav[item].stamp <= data.fav[fav].stamp) {
                                            valid = true;
                                        }
                                    }
                                    for(let list in data.fav[fav].list) {
                                        let valid2 = true;
                                        for (let element in localFav[item].list) {
                                            if (localFav[item].list[element].id == data.fav[fav].list[list].id) {
                                                if (localFav[item].stamp <= data.fav[fav].stamp) {
                                                    valid2 = true;
                                                }
                                            }
                                        ...
                                        if(valid2) {
                                            localFav[item].list.push(data.fav[fav].list[list]);
                                            check = false;
                                        }
                                    }
                                }
                                if (valid && check) {
                                    localFav.push(data.fav[fav]);
                                }
                            }
                        ...
                        try{
                            ...
                            if(myRadio.sync.checkCallSign(data.sign)){
                                ...
                                if(myRadio.sync.decode(data.message) ==  myRadio.sync.askForData){
                                    // Send dat broadcast
                                    let sendData = myRadio.sync.getDataToSync();
                                    // Split in more than 1 because of big data ------------------
                                    // And send items
                                    // for Login:
                                    if(sendData.login.length >= 4) {
                                        for (let i = 0; i < sendData.login.length; i = i + 3) {
                                            let newData = {
                                                fav: null,
                                                login: [
                                                    sendData.login[i],
                                            ...
                                            myRadio.sync.send(JSON.stringify(newData));
                                        }
                                    }else{
                                        myRadio.sync.send(JSON.stringify({
                                            fav: null,
                                            login: sendData.login
                                        }));
                                    }
                                    // Fav
                                    console.log(sendData.fav[0].list.length );
                                    if(sendData.fav[0].list.length >= 4) {
                                        for (let i = 0; i < sendData.fav[0].list.length; i = i + 3) {
                                            let newData = [{
                                                login: null,
                                                fav: [{
                                                    list: [
                                                        sendData.fav[0].list[i],
                                                        sendData.fav[0].list[i + 1],
                                                        sendData.fav[0].list[i + 2]
                                                    ],
                                                    stamp: sendData.fav[0].stamp,
                                                    id: sendData.fav[0].id
                                                }]
                                            }];
                                            myRadio.sync.send(JSON.stringify(newData));
                                        }
